...
export default function Services() {
  const tBase = useTranslations('services');
  const locale = useLocale();
  const t = useSectionT('services', tBase, locale);
  const { services: apiServices } = useContent();
  const services = apiServices.length > 0 ? apiServices : staticServices;
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', flex: 1 }}>
                    <h3 style={{
                      ...
                      {s.translations?.[locale]?.title || s.translations?.ru?.title || tBase(`items.${sId}.title`)}
                    </h3>
                    <p style={{
                      fontSize: '0.875rem',
                      ...
                      {s.translations?.[locale]?.subtitle || s.translations?.ru?.subtitle || tBase(`items.${sId}.desc`)}
                    </p>
                  </div>
